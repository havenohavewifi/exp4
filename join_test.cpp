...
*/
 ...
 /*
  ...
  /*  insertOneTuple(&head, "customer", "501|Customer#000000501|IVhzIApeRb ot,c,E|15|25-989-741-2988|711.56|BUILDING|to the even, regular platelets.H|");
    ...
    if (customer_scan>=0) {
        printf("tablescan customer succeed!\n");
    }else{
		 printf("tablescan customer failed!\n");
	}
	int nation_scan = -1;
   nation_scan = TableScan(&head, temp_data_dict, "nation");
    if (nation_scan>=0) {
        printf("tablescan nation succeed!\n");
    }
	else{
		 printf("tablescan nation failed!\n");
	}
	//if(true == createIndexOn(&head, FIRST_FID, "nationkey")){
       // char* index_filename = "/Users/Irene/Desktop/b_plus_tree_index_1custkey.dat";
	  /*  char* index_filename = "./b_plus_tree_index_1Nationkey.dat";
        FILE* fp;
        int pos;*/
	 int goal=-1;
	for (int i = 0; i<MAX_FILE_NUM; i++) {
		if (strcmp((head.redef)[i].getRelationName(),temp_data_dict[customer_scan].getRelationName())==0) {
				goal=i;
				break;
			}
	}
	...
	cout<<"goal:"<<goal<<endl;
	//if(true == createIndexOn(&head, actual, "nationkey")){
	...
	//hashjoin(&head,temp_data_dict,nation_scan,customer_scan,"nationkey");
	/* if(true == createIndexOn(&head, FIRST_FID, "nationkey")){
		 head.redef[goal].getAttributeByName("nationkey").changeIndexed((bool)1);
		cout<<"is indexed:"<< head.redef[goal].getAttributeByName("nationkey").isIndexed()<<endl;
	}else{
		cout<<"error"<<endl;
	 }*/
	//int actual=(head.desc).fileDesc[goal].fileID;	
	//cout<<"actual:"<<actual<<endl;
	//cout<<"fid:"<<FIRST_FID<<endl;
	/*if(true == createIndexOn(&head, FIRST_FID, "nationkey")){
		
	}
	*/	//join(&head,temp_data_dict,nation_scan,customer_scan,"nationkey");
		//indexjoin(&head,temp_data_dict,nation_scan,customer_scan,"nationkey");
       ...
       /*if( !(fp = fopen(index_filename,"rb+")))
		...
		pos = searchByColumnAndValue(&head,FIRST_FID,"custkey",1);
		int rec_len_ = temp_data_dict[customer_scan].getRecordLength();
		char* one_Row_ = (char *)malloc(sizeof(char)*rec_len_);
		rdFile(& head, 0, FIRST_FID, pos, rec_len_,one_Row_);
		getOneRecord(one_Row_, &(temp_data_dict[customer_scan]));*/
...
//cout<<nestloop_cp_new(&head, temp_data_dict,customer_scan,nation_scan);
